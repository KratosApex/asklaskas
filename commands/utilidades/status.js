    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            const fetchAndDisplayStatus = async (editReply = true) => {
                try {
                    const response = await fetch(`http://${client.config.HOST_FIVEM}:${client.config.PORT_FIVEM}/players.json`);
                    const serverInfoResponse = await fetch(`http://${client.config.HOST_FIVEM}:${client.config.PORT_FIVEM}/info.json`);

                    // Verificar se o servidor está online
                    if (!response.ok || !serverInfoResponse.ok) {
                        const embedOffline = new EmbedBuilder()
                            .setColor(0xFF0000)
                            .setTitle('Servidor Offline')
                            .setDescription('O servidor está offline ou inacessível no momento.')
                        ...
                        return { embed: embedOffline, components: [] };
                    }
                    ...
                    if (isPublic) {
                        ...
                        const privateEmbed = new EmbedBuilder()
                            .setColor(0x0099FF)
                            .setTitle(`Status do Servidor - ${serverInfo.vars.sv_hostname || 'Servidor FiveM'}`)
                            .setDescription(`Informações atualizadas em `)
                            .addFields(
            ...
            await interaction.editReply('❌ Ocorreu um erro ao obter o status do servidor.');
        }
    },
};
